# Wagerbot (README WIP)
#### VIDEO: https://youtu.be/6NdPzwxJKzM?si=7UMcS1f102Prf8N1
## Overview
...
* `/declare_winner` completes the wager and transfers the funds from the loser's account to the winner's. It uses `check_ongoing` on both the command user and the mentioned user to see if they even have any ongoing wagers; if they do, `check_match` is used to check if the wager that they're in, which is "Ongoing", shares the same wager ID. Once that is done, it uses `declare_winner` to transfer the funds between users, changes the wager status to "Resolved", and inputs the winning user into the winner ID slot of the wager.
* `/user_info` sends a DM which includes a basic account overview and a link to your account info page on wagerbot.ca. First, it runs `profile_checker` to check if you even have an account. Then it grabs your token from the database, which is used to generate your link. After that, `get_basic` is used to grab your basic user info of wins, losses, balance, and username; all of this is formatted as a dictionary. That dictionary is used to format a string which displays your info in text form for a Discord message. It also uses `check_ongoing` and `check_pending` to see if it needs to display any info relating to a wager you're in. Your wager info is also formatted into a string which displays the opponent, amount, status, and the note created by whoever created the wager. Lastly, the link to your account page is added, then the whole message is sent. Once sent, the bot will send a message in the thread you've interacted with it, informing you that your information is now available. 
